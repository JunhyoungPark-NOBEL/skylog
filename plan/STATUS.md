...
| T2 센서 연동(AR 모드) | 🟡 구현 완료 · **실기기 검증 대기** | | (task-2-done은 실기기 반영 후) | 시뮬레이터 e2e 통과. iOS 3-자세 결과로 compassAxis·편각 확정 |
...
- **T2는 코드·시뮬레이터 검증까지 끝났고 실기기 결과만 남았다.** 사용자가 `docs/TESTING.md`의 T2 체크리스트(특히 iOS 3-자세 덤프)를 주면: (1) iOS `webkitCompassHeading`이 상단 축인지 후면 축인지 → `sensorStore.compassAxis` 기본값 확정, (2) iOS heading에 편각을 더해야 하는지(자북 가설) 확인, (3) 지연·떨림 체감으로 `DEFAULT_FILTER` 상수 조정, (4) `task-2-done` 태그. 결과가 오기 전이라도 **T3a는 시작 가능**(센서와 독립).
