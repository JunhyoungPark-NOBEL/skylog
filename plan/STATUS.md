...
## 최근 작업 보고 (2026-09-13 · 시야 고정·목표 취소 / build30)

- 최신 main/origin9deeff2 확인, install/type·기존 단위798개 통과. 사용자에게 이전 구현의 작은 출력 차단 범위와 지속 추종이 잔여 흔들림을 남긴 원인을 설명했다.
- 육안/망원경 상단 한 줄 목표를 강조색으로 표시하고44px 취소 X를 항상 노출한다. 빠른 검색 진입에서 천체 이름이 내부 ID로 남는 오류도 수정했다.
...
- 자동 수용6항목 확인, 실기기 정확도1항목 대기. 타입/lint·단위798개 통과. 브라우저 전체114개 중112개 통과 후 검색 갱신 대기와 이전 배치 검사를 수정했고, 해당7개 모두 통과했다. 200% 글자에서 새 HUD의 터치 높이가 불필요하게 커지는 부분과 망원경 모드의 중복 지평선 문구도 시각 검사로 보완했다. 관련17개 중16개 통과 후 목표 버튼의 인라인 여백을 줄여 남은1개도 최종 통과했다. 모든 실패 항목을 재검증했으며 배포 결과는 아래와 같다.
...
- **최종 배포/산출물**: 소스 `01afbd25f5f0916b644cead561547bbd49e4b778`, 태그 `v0.1.0-beta.23-build29`. [Pages34736379970](https://github.com/JunhyoungPark-NOBEL/skylog/actions/runs/34736379970)·[모바일34736379941](https://github.com/JunhyoungPark-NOBEL/skylog/actions/runs/34736379941) 성공. Android release/lint 오류0(기존 경고32)·API36 오프라인2개·iOS 컴파일 통과, 원본 XML도 확인했다. 공개 웹200·목표 한 줄/파인더 보정·JS오류0·서버쓰기0. 무서명 AAB20,790,368bytes, SHA256 `98c691dd7119c7b29f4cb263db5715ddab34764ece4cd3cf4847787d438d1d78`; bundletool·정적파일480개 비교 통과. 서명 인계 ZIP8항목 및 내부 AAB 해시 검증. 실제 S24+ 보정 정확도/드리프트와 연구실 동일 인증서 서명은 남는다.

## 이번 작업 보고 (2026-09-13 · GPS 방향 안정화 / build28)

- 최신 main/origin6f358ad 확인. GPS에 나침반 아이콘, 광각·확대 두 단계 안정화, 실제 측정 시각 전달과40ms 렌더 재표본화를 적용했다.
- Android 자동 추적은 자이로 자세와8초 북쪽 보정을 분리한다. 기준 없는 상대 방위를 북쪽처럼 노출하지 않으며 기존 자이로·+Y 경통·별 정렬/수동 동선을 보존한다.
- typecheck·lint·단위787개·PWA 빌드·전체 브라우저113개 통과. 합성60Hz 입력의 별 RMS는90°에서0.476→0.241px,3°에서2.895→0.794px. 이는 S24+ 실측치가 아니다.
- D-074, [수치/설계/실기기 보고](../docs/SENSOR-STABILITY-BUILD28.md). 실행 소스a8e3146, 태그v0.1.0-beta.22-build28, Pages34733709128·모바일34733709370 성공. 공개 웹200/나침반·동선, AAB/bundletool·480파일·Android 오프라인2개·iOS 컴파일 통과. 기존 연구실 Play 키 최종 서명과 S24+ 실제 사용감 확인은 남는다.
...
- **build30(D-076)**: IntentStabilizer는 CameraController의 표시 전용이다. 이것을 측정 필터/세 별 캘리브레이션 입력에 적용하지 않는다. 고정 각도는0.325~1.05°로 절대 포인팅 오차와 구분한다. 합성 시험에서 정지했어도 S24+ 실측을 주장하지 않는다. 실기기 체크는 docs/INTENT-STABILITY-BUILD30.md, 이전 build29 설치 보정 조건도 유지한다. 목표 X는 항상 표시한다(D-075의 메뉴 안 종료 정책을 최신 사용자 요청으로 변경).

- build28은 광각·확대 공통 필터와 Android 자이로/북쪽 융합을 변경했다. 비교 수치는 합성 시험이며 S24+ 실기기 결과는 아직 없다. 새 방향 수집에는 측정 timestampMs를 끝까지 유지하고 재연결/상대 프레임을 보존한다. 자세 재표본화40ms를 React 갱신 제한과 혼동하지 않는다.
...
- **build26 유지 동선**: 최신 실기기 확인은 SENSOR-STABILITY-BUILD28.md를 따른다. 하늘 진입은 #/sky?scope=ObjectId, 스타호핑은 #/telescope?view=hop&course=기존ID. 필터의 setViewport는 실제 FOV/화면 픽셀을 전달하며 줌 변경 때 reset하지 않는다. 센서 trackingMode는 compass/gyro, manualPauseUntil은 호환 이름을 남긴 런타임 플래그(1=사용자 버튼 대기)이며 타이머가 아니다. 자이로 상대 보정은 재시작 시 재사용하지 않는다.
- **집 PC와 연구실 키 구별**: 기존 문서의 “이 PC”는 당시 연구실을 뜻한다. 현재 PC에는 이전 업로드 키만 있다. build28의 기존 Play 서명은 별도 인계가 필요하며 scripts/sign-play-update.ps1이 AAB 해시와 build24 인증서를 검증한다.

- **기존 기능**: beta.20/build26 확대 안정화·메인 하늘 안내(D-072), beta.18/build24 천체물리 서사 개편(D-070), 이전 풍경·업적(D-069)은 보존한다. 새 선택적 backdrop은 migration202609110002와 함께 사용한다. 기존 Play/개인 APK 키를 재사용하며 최종 검사·산출물·배포 상태는 문서 맨 위와 docs/MOBILE-RELEASE.md를 따른다.
...
- **최신 검증**: 문서 맨 위 build28 보고를 따른다. 이전 버전 수치는 당시 이력이며 실제 휴대폰 센서/성능·일반 SMTP·스토어 심사·Apple 서명은 별도다.
...
build25 보고는 [보관 기록](reports/build25-field-use-summary.md)을 참고한다. build23 보고는 [보관 기록](reports/2026-09-11-build23.md)을 참고한다. 이전 T3b/T4/T5/T6·학습 UX 보고는 [기능 개발 보관 기록](reports/2026-09-08-pre-mobile-history.md)을 참고한다.
