...
- **Play 데이터 삭제 URL 보완**: `delete-account.html#delete-data`에 계정을 유지한 공유 사진·개인 백업 삭제의 실제 UI 경로, 웹/이메일 요청, 삭제·유지 범위를 한영으로 추가했다(D-062). 사용자는 광고를 넣지 않기로 확정했다. 정적 안내 변경으로 build17 AAB/APK는 재생성하지 않으며 공개 배포 확인은 아래 후속 기록을 참고한다. 서버 로그/검토 이력 보관 기간은 기존 미확정 사항으로 남기고 임의 기간을 약속하지 않는다.
