...
test('선행 문제는 오답 재도전과 두 정답을 저장하고 본 문제 점수를 올리지 않는다', async ({
  page,
}) => {
  const errors: string[] = [];
  ...
  for (let i = 0; i < lesson.warmups.length; i++) {
    ...
    await prep
      .getByRole('button', {
        name: i + 1 < lesson.warmups.length ? /다음 준비 문제/ : /이제 본 문제 풀기/,
      })
      .click();
  }
...
test('준비 문제 건너뛰기는 완료로 기록되지 않고 새로고침하면 준비 문제를 다시 제시한다', async ({
  page,
}) => {
  await page.goto(path);
  await page
    .getByTestId('history-preparation')
    .getByRole('button', { name: /본 문제로/ })
    .click();
  await expect(page.getByTestId('history-main-content')).toBeVisible();
  ...
  await expect(page.getByTestId('history-main-content')).toBeHidden();
  await expect(page.getByTestId('history-preparation')).toContainText('1/2');
  await expect(page.getByTestId('history-result')).toHaveCount(0);
});
...
test('영어 야간 360px·125% 글자에서 본 문제 관계식의 아래첨자가 직립체이고 가로로 넘치지 않는다', async ({
  ...
  await page.getByRole('button', { name: 'Problem 2', exact: true }).click();
  await page
    .getByTestId('history-preparation')
    .getByRole('button', { name: /Skip to challenge/ })
    .click();
