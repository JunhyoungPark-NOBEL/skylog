...
const SATURN: Candidate = { id: 'planet:saturn', kind: 'planet', bodyKey: 'saturn', raJ2000Deg: 0, decJ2000Deg: 0, extended: false };
const MOON: Candidate = { id: 'moon', kind: 'moon', bodyKey: 'moon', raJ2000Deg: 0, decJ2000Deg: 0, extended: false };
const VEGA: Candidate = { id: 'star:HIP91262', kind: 'star', raJ2000Deg: 279.2346, decJ2000Deg: 38.78369, mag: 0.03, extended: false, con: 'Lyr' };
const M31: Candidate = { id: 'dso:M31', kind: 'dso', raJ2000Deg: 10.68479, decJ2000Deg: 41.26906, mag: 3.44, majArcmin: 177.83, minArcmin: 69.66, extended: true, category: 'galaxy', con: 'And' };
const M13: Candidate = { id: 'dso:M13', kind: 'dso', raJ2000Deg: 250.4235, decJ2000Deg: 36.4613, mag: 5.8, majArcmin: 20, minArcmin: 20, extended: true, category: 'globularCluster', con: 'Her' };
const M42: Candidate = { id: 'dso:M42', kind: 'dso', raJ2000Deg: 83.82208, decJ2000Deg: -5.39111, mag: 4, majArcmin: 90, minArcmin: 60, extended: true, category: 'nebula', con: 'Ori' };
const M101: Candidate = { id: 'dso:M101', kind: 'dso', raJ2000Deg: 210.8023, decJ2000Deg: 54.3489, mag: 7.9, majArcmin: 24, minArcmin: 23, extended: true, category: 'galaxy', con: 'UMa' };
const SIRIUS: Candidate = { id: 'star:HIP32349', kind: 'star', raJ2000Deg: 101.28716, decJ2000Deg: -16.71612, mag: -1.44, extended: false, con: 'CMa' };
...
describe('추천 엔진', () => {
  ...
  it('도시(Bortle 8)에서 어두운 은하(M101)는 망원경 그룹에서도 순위가 낮고 맨눈 그룹에는 없다', () => {
    ...
    if (tel.includes('dso:M101')) expect(tel.indexOf('dso:M101')).toBeGreaterThan(tel.indexOf('dso:M13'));
  });
  it('달 근접 감점: 같은 대상이라도 달이 밝고 가까우면 점수가 내려간다', () => {
    ...
    const r2 = recommend(base({ now: t2, night: n2, window: { from: n2.darkSpan!.from, to: new Date('2026-09-25T16:00:00Z') }, candidates: [VEGA] }));
  ...
  it('관측지 범위(visibleAz) 하드 필터: 동쪽만 보이면 서쪽 대상이 빠진다', () => {
    ...
    const east = recommend(base({ candidates: [VEGA, SATURN], site: { visibleAz: [[45, 135]], bortle: 7 } }));
    ...
    const west = recommend(base({ candidates: [VEGA, SATURN], site: { visibleAz: [[200, 320]], minAltDeg: 20, bortle: 7 } }));
  ...
  it('구름 ≥ 70%인 시간은 창에서 제외된다', () => {
    ...
    const r2 = recommend(base({ candidates: [VEGA], cloudAt: (t) => (t.getTime() < new Date('2026-09-06T13:00:00Z').getTime() ? 90 : 10) }));
    expect(r2.items.length).toBe(1);
    expect(r2.items[0]!.metrics.firstVisibleAt!.getTime()).toBeGreaterThanOrEqual(new Date('2026-09-06T13:00:00Z').getTime());
  });
  it('그룹: 지금 당장·곧 진다·올라오는 중, 계획은 시간순 ≤ 12', () => {
    const r = recommend(base({ equipment: 'binoculars', events: [{ objectId: 'planet:saturn', kind: 'opposition', at: new Date('2026-10-04T00:00:00Z'), withinDays: 30 }], seasonSignatures: SEASON_SIGNATURES }));
    expect(r.groups.now.length).toBeGreaterThan(0);
    expect(r.plan.length).toBeLessThanOrEqual(12);
    for (let i = 1; i < r.plan.length; i++)
      expect(r.plan[i]!.metrics.peakAt!.getTime()).toBeGreaterThanOrEqual(r.plan[i - 1]!.metrics.peakAt!.getTime());
  ...
  it('신선도: observedSet에 없는 대상만 보너스', () => {
    ...
    const b = recommend(base({ candidates: [VEGA], observedSet: new Set(['star:HIP91262']) })).items[0]!.score;
    expect(a - b).toBe(5);
  });
});
...
describe('실제 하늘처럼 한계등급', () => {
  it('Bortle과 달에 따라 내려간다, 최소 2', () => {
    ...
    expect(realSkyLimitingMag({ bortle: 8, moonIllumination: 1, moonAltDeg: 60 })).toBeLessThan(3.5);
    expect(realSkyLimitingMag({ bortle: 9, moonIllumination: 1, moonAltDeg: 60 })).toBeGreaterThanOrEqual(2);
  });
});
