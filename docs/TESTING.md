...
## build30 시야 고정·목표 취소 (2026-09-13)

- type/lint·단위813개(117파일)·PWA 빌드. 전체 브라우저115개 중114개 통과 후 이름 초기화 오류 수정, 관련8개 재검증 통과. 실제 시뮬레이터→스토어→렌더 루프 추가 통합1개도 통과(총116개).
- 정지 범위·작은 왕복·이동/반전/정지·불규칙30/60/90Hz→120Hz·광각/확대·천정 근처·별 선택과 센서 유지·재연결·기존3별 보정 동선을 검사한다.
- 정상/야간 목표 대비4.5:1 이상, X44px,360px/글자200% 스크린샷 직접 확인. 새 체크리스트와 합성 수치의 한계는 [build30 보고](INTENT-STABILITY-BUILD30.md).
...
- 브라우저: 9월/10월 버튼·월간 선택/키보드·12개월 연간·ICS 실제 다운로드·9월30일→10월1일 자동 전환·목록 복귀·360px 영어125% 글자. 날씨 기본 접힘/상세/실패, 3테마→스타호핑→기록→진도 복원. 지평선0°/−14°/−35°의 실제 재질·풍경 로드/비활성 및 야간 캡처.
- 전체 검사에서 센서 끄기 뒤 즉시 새로고침 시 Dexie 저장보다 탐색이 앞서는 1건이 있었다. 검사에서 실제 sensor.autoStart=false 행의 트랜잭션 완료를 관찰하도록 수정했다. 센서 구현은 바꾸지 않았으며 임의 sleep/자동 재시도를 추가하지 않았다.
- 폰 체크리스트: [개선 기록](TONIGHT-REFRESH.md). API36 네트워크 차단 계측도 새 코스 경로와 풍경의 번들 포함을 확인한다.
