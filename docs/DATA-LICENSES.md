...
`public/object-photos/v1/`의 사진은 개별 ESA/Hubble·ESO 공식 이미지 페이지와 권리 정책을 확인한 CC BY 4.0 자료다. 이 사진들은 위의 카탈로그 데이터 팩 CC BY-SA와 별도 라이선스이며, 코드의 MIT 라이선스로 재허가하지 않는다. 로고·삽화·권리 예외 자료는 포함하지 않는다.

각 사진의 원문 크레딧·연결된 저작자 링크·출처·CC BY 4.0 링크는 검색 미리보기, 선택 카드, 상세 화면 가까이에 표시한다. 앱의 ‘정보/라이선스’에도 활성 링크가 있는 전체 목록을 제공한다. WebP 변환·종횡비 유지 축소와 야간 보호의 적색 표시를 알리고, 합성 파장·대상 일부/넓은 영역 여부는 실제 관측 정보에 맞게 설명한다. 현재 실시간 모습이나 실제 접안 시야를 재현한 사진으로 안내하지 않는다.
