...
검토일: 2026-09-11. 콘텐츠 버전: `HISTORY_QUESTS_VERSION = 1`.
...
beta.18에서는 `historyNarrative.ts`에 열 이야기의 도입·질문·90개 장면·30개 발견 기록·10개 결말을 한영으로 작성했다. 원문 문장을 옮기거나 실제 대화·일기를 꾸며 인용하지 않는다. 독자가 관측표·건판·지도·회전 곡선을 읽는 장면은 역사에 기반한 교육용 서술이며 계산은 기존 모형을 사용한다. 역사 원자료와 예시 수치를 섞지 않도록 장면 자체에도 현대 보정·독립된 자료·모형 전환을 표시했다.

파리 천문대의 뢰머 설명, Harvard Plate Stacks의 건판 비교·리비트 설명, Harvard Wolbach의 페인 학위논문 소개, NASA 케플러 설명, Zwicky 원논문 번역, APS 에라토스테네스 설명을 재검토했다. 찬드라세카르·허블·루빈·일식 편의 기존 근거와 수학적 제한도 유지했다. 결말은 세 계산·판단을 각각 한 번 제출하면 열리며, 정답 판정·독립 풀이·보상을 바꾸지 않는다. 장면을 건너뛰거나 초안만 작성한 것으로는 결말이 열리지 않는다. 구버전 풀이 기록도 같은 조건으로 인정한다.
