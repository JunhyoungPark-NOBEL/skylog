...
> **beta.9 추가 확인**: 천체 관측 사진20개는 개별 CC BY 4.0 출처/전체 크레딧/변환 고지를 포함한다. 사진 자체는 번들에 포함되고 외부 이미지 서버로 조회하지 않는다. 스토어 스크린샷에 사진을 사용할 때도 해당 전체 크레딧을 유지한다. 댓글 기능의 일반 가입·운영 개통은 대기이며, 심화 학습 유료화는 미래 계획으로 현재 결제/잠금이 없다. 최신 검토: OBJECT-PHOTOS.md, COMMENTS-REVIEW.md, MONETIZATION-PLAN.md.
