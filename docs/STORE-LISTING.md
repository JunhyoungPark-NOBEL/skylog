...
> **beta.10 추가 확인**: 관측 사진 목록을 확대하고 작은 미리보기의 긴 출처를 자세히 화면으로 옮겼다. 각 사진의 원문·제작자·이용 조건·변환 기록은 OBJECT-PHOTOS.md와 사진 manifest에 있다. 사진은 번들에 포함되며 외부 이미지 서버로 조회하지 않는다. 스토어 스크린샷에 사진을 사용할 때는 해당 사진의 개별 이용 조건을 확인한다. 로그인 링크 복구는 AUTH-LOGIN.md를 참고한다. 일반 회원 메일 발송·커뮤니티 운영 개통은 대기이며, 심화 학습 유료화는 미래 계획으로 현재 결제/잠금이 없다.
