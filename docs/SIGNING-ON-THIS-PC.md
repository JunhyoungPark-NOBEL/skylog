...
최종 제출 파일은 Downloads의 build17 폴더에 있는 **`skylog-0.1.0-beta.12-build17-play-signed.aab`**다. 서명 검증 결과와 SHA는 [MOBILE-RELEASE](MOBILE-RELEASE.md)에 기록한다.
...
3. 서명된 AAB를 내부 테스트 릴리스에 올리고 버전 코드17·API36·업로드 인증서 SHA256을 대조한다.
4. 등록이 끝나면 이후 AAB도 이 업로드키를 유지한다. Console 등록 성공은 현재 로컬 서명 성공과 별도이며 아직 수행하지 않았다.
