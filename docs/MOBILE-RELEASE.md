...
2026-09-08 기준. 앱 ID 기본값은 `io.github.junhyoungparknobel.skylog`, 현재 소스 버전은 `0.1.0-beta.2`(iOS marketing 0.1.0), 이번 산출물의 빌드 번호는 **7**이다. 이 ID는 저장소 소유자에서 정했으며 **스토어 등록·최종 앱 ID·기존 업로드 키는 계정 소유자의 확인이 필요하다**. T5 실기기 승인과 T7/T8 전체 완료를 의미하지 않는다.
...
- [ ] 공개 배포 완료 후 iPhone Safari/홈 화면 PWA의 설정 버전이 beta.2인지 확인하고 자동 안내·원형 하늘·상세 창 스와이프와 기존 기록 보존을 확인한다. PWA 테스트와 TestFlight 네이티브 테스트를 구분한다.
