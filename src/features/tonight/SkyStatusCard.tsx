  ...
  return (
    ...
    >
      <div className="flex items-baseline justify-between">
        <h2 className="text-base font-semibold">{t('tonight.skyStatus')}</h2>
        <span className="text-xs text-muted">{t('tonight.night', { date: dateLabel })}</span>
      </div>
      <svg
        viewBox={`0 0 ${W} ${H}`}
        ...
        {hours.map((ms) => (
          <g key={ms}>
            <line
              ...
              strokeWidth={1}
            />
            <text x={scale.x(ms)} y={H - 10} textAnchor="middle" fontSize={13} fill="var(--muted)">
              {hourLabel(ms)}
            </text>
          </g>
        ))}
        ...
        {now.getTime() >= scale.from && now.getTime() <= scale.to && (
          <g data-testid="now-cursor">
            <line
              ...
              strokeWidth={2}
            />
            <text x={scale.x(now)} y={PAD + 76} textAnchor="middle" fontSize={12} fill="var(--fg)">
              {t('tonight.now')}
            </text>
          </g>
        )}
      ...
      <div className="mt-1 flex gap-3 text-[11px] text-muted">
        ...
        <span>
          <span
            className="mr-1 inline-block h-2 w-3 rounded-sm"
            style={{ background: 'var(--moon)', opacity: 0.6 }}
          />
      ...
      <div className="mt-3 grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
        {rows.map((r) => (
          <div key={r.testId} className="flex justify-between">
            <span className="text-xs text-muted">{r.label}</span>
            <span className="font-mono" data-testid={`status-${r.testId}`}>
              {r.value}
            </span>
          </div>
        ))}
      ...
      <div className="mt-3 flex items-center justify-between rounded-xl bg-surface-2/60 px-3 py-2 text-sm">
        <span className="font-semibold" data-testid="status-moon-phase">
          {moonName}
        </span>
      ...
      <div className="mt-2 rounded-xl bg-surface-2/60 px-3 py-2 text-sm">
        <div className="flex items-baseline justify-between">
          <span className="font-semibold">{t('tonight.darkWindow')}</span>
          <span className="text-xs text-muted">{t('tonight.darkWindowHint')}</span>
        </div>
        {night.darkWindows.length === 0 ? (
          <p className="mt-1 text-xs text-muted" data-testid="dark-none">
            {t('tonight.darkNone')}
          </p>
        ) : (
          <ul className="mt-1 space-y-0.5" data-testid="dark-list">
            {night.darkWindows.map((w, i) => (
            ...
            <li className="text-right text-xs text-muted">
