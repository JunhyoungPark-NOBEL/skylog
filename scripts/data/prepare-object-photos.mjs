...
const sourceCache = path.join(root, 'artifacts/object-photos-research/originals-v2');
const prefetchCache = path.join(root, 'artifacts/photo-prefetch15');
...
const licenses = new Set(['Public domain', 'NASA/JPL image use', 'CC BY 4.0']);
function xmp(photo) {
...
dc:source="${xml(photo.sourceURL)}" dc:creator="${xml(photo.credit)}" dc:rights="${xml(photo.license)}" xmpRights:WebStatement="${xml(photo.licenseURL)}"
skylog:objectId="${xml(photo.objectId)}" skylog:sourceURL="${xml(photo.sourceURL)}" skylog:imageURL="${xml(photo.imageURL)}" skylog:credit="${xml(photo.credit)}" skylog:licenseURL="${xml(photo.licenseURL)}" skylog:rightsURL="${xml(photo.rightsURL)}" skylog:rightsEvidence="${xml(photo.rightsEvidence.en)}" skylog:verifiedAt="${xml(photo.verifiedAt)}" skylog:sourceSha256="${xml(photo.sourceSha256)}" skylog:sourceDimensions="${xml(JSON.stringify(photo.sourceDimensions))}" skylog:crop="${xml(JSON.stringify(photo.crop ?? null))}" skylog:modifications="${xml(photo.modifications.en)}" skylog:caption="${xml(photo.caption.en)}"/>
...
function assertRecord(photo) {
  ...
  if (
    !licenses.has(photo.license) ||
    !photo.verifiedAt ||
    !photo.rightsEvidence?.ko ||
    !photo.rightsEvidence?.en ||
    new URL(photo.imageURL).protocol !== 'https:' ||
    !trustedHosts.has(new URL(photo.imageURL).hostname)
  )
    throw new Error(`Unreviewed source: ${photo.objectId}`);
}
function assertGeometry(photo) {
  const dimensions = photo.sourceDimensions;
  if (
    !dimensions ||
    ...
    throw new Error(`Missing source dimensions: ${photo.objectId}`);
  if (!photo.crop) return;
  const { left, top, width, height, reason } = photo.crop;
  if (
    ...
    !['editorial-frame', 'observation-panel'].includes(reason)
  )
    throw new Error(`Invalid reviewed source crop: ${photo.objectId}`);
}
async function generate(photo) {
  assertRecord(photo);
  if (!process.argv.includes('--force') && photo.hero?.sha256 && photo.thumb?.sha256) {
    try {
      await verify(photo);
      console.log(`${photo.objectId}: verified existing WebP pair`);
      return;
    } catch {
      // 출처·크레딧·가공 메타데이터가 달라졌거나 파일이 없으면 다시 만든다.
    }
  }
  const slug = photo.objectId.replaceAll(':', '-').toLowerCase();
  // 천체의 사진을 바꿔도 이전 출처의 캐시를 잘못 재사용하지 않는다.
  const cachePath = path.join(sourceCache, `${sha256(photo.imageURL)}.image`);
  let source;
  try {
    ...
    try {
      source = await readFile(path.join(prefetchCache, `${sha256(photo.imageURL)}.image`));
    } catch (prefetchError) {
      if (prefetchError.code !== 'ENOENT') throw prefetchError;
    }
  }
  if (!source) {
    if (process.argv.includes('--offline-inputs'))
      throw new Error(`Reviewed source cache missing: ${photo.objectId}`);
  ...
  if (sourceMetadata.orientation && sourceMetadata.orientation !== 1)
    throw new Error(`Review source orientation before using pixel coordinates: ${photo.objectId}`);
  const dimensions = { width: sourceMetadata.width, height: sourceMetadata.height };
  if (
    photo.sourceDimensions &&
    (photo.sourceDimensions.width !== dimensions.width ||
      photo.sourceDimensions.height !== dimensions.height)
  )
    throw new Error(`Source dimensions changed: ${photo.objectId}`);
  photo.sourceDimensions = dimensions;
  assertGeometry(photo);
  ...
  for (const [variant, maxSide, quality] of [
    ...
    if (photo.crop) {
      ...
      input = input.extract({ left, top, width, height });
    }
...
async function verify(photo) {
  ...
  for (const [variant, limit] of [
    ...
    const scale = Math.min(1, limit / frame.width, limit / frame.height);
    if (
      Math.abs(asset.width - Math.round(frame.width * scale)) > 1 ||
      Math.abs(asset.height - Math.round(frame.height * scale)) > 1
    )
      throw new Error(`Reviewed frame was not preserved during resize: ${asset.path}`);
  }
}
if (!checkOnly) {
  ...
  manifest.preparedWith = {
    ...
    heroQuality: 76,
    thumbQuality: 72,
    crop: 'per-record reviewed editorial-frame or whole observation-panel only',
  };
...
const listed = new Set(
  manifest.photos.flatMap((photo) => [
    path.basename(photo.hero.path),
    path.basename(photo.thumb.path),
  ]),
);
for (const file of await readdir(folder)) {
  if (!file.endsWith('.webp') || listed.has(file)) continue;
  if (checkOnly || !process.argv.includes('--prune'))
    throw new Error(`Unlisted photo asset: ${file}`);
  // 제거 대상은 이 고정 public 폴더의 단일 WebP만 허용하고 원본 사본을 보존한다.
  const target = path.resolve(folder, file);
  if (path.dirname(target) !== folder || !/^[a-z0-9-]+\.webp$/.test(file))
    throw new Error('Unsafe unlisted photo path');
  const retired = path.join(root, 'artifacts/object-photos-research/retired-v1');
  await mkdir(retired, { recursive: true });
  await copyFile(target, path.join(retired, file));
  await unlink(target);
  console.log(`Retired unlisted photo: ${file}`);
}
const bytes = manifest.photos.reduce((sum, photo) => sum + photo.hero.bytes + photo.thumb.bytes, 0);
if (bytes > 15_000_000) throw new Error(`Photo budget exceeded: ${bytes} bytes`);
console.log(
  `Verified ${manifest.photos.length} photos / ${manifest.photos.length * 2} WebP files / ${bytes} bytes. Reviewed frame crops only; no upscaling.`,
);
